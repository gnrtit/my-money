...
모든 코드/파일 수정 작업이 끝나면 반드시 아래 순서로 처리한다:

1. **README.md 업데이트**: 변경된 기능이 README에 반영되어야 하면 업데이트한다.
2. **CHANGELOG.md 업데이트**: 없으면 새로 만들고, 있으면 맨 위에 항목을 추가한다.
   - 형식: `## [날짜] - 변경 제목` + 변경 내용 bullet
   - 날짜는 `YYYY-MM-DD` 형식 사용
3. **커밋**: 반드시 직접 의미 있는 메시지로 커밋한다 (Stop 훅의 `auto:` 커밋에 맡기지 않는다).
   - 제목: `타입: 한 줄 요약` (예: `feat: 설정 탭 추가`, `fix: 대시보드 계산 오류 수정`)
   - 본문: 무엇을 왜 바꿨는지, 주요 변경 파일/함수 명시
   - 타입: `feat`(신기능) `fix`(버그) `refactor`(리팩터) `docs`(문서) `chore`(설정)
4. **푸시**: Stop 훅이 자동으로 처리하므로 별도 실행 불필요.
